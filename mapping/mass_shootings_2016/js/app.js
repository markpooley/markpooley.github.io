...
queue()
  ...
  .defer(d3.json, 'data/mass_shootings_2016.topojson')
  .await(makeMap);
...
function makeMap(error, states, shootings){
 ...
 // })
  firstIncident = dateFormat.parse('January 1, 2016')
  ...
  incidentGraph.selectAll('g').select('#xAxis').append('text')
    .text('Jan')
    .attr('transform','translate('+margin.left+','+(graphHeight-margin.bottom)+')')
